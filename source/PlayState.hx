...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'chaos':
				dialogue = CoolUtil.coolTextFile(Paths.txt('chaos/dialog'));
		}
			...
			case 'void':
				{
						defaultCamZoom = 0.7;
						curStage = 'void';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('voidBg'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
	
						var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('voidPlatform'));
						stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
						stageFront.updateHitbox();
